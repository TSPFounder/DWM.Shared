...
namespace DWM.Shared.Tooling.Cad
{
    public static class FusionScripts
    {
        ...
        ///
        /// bodyCount IS PART OF THE CONTRACT, not a nicety. A component holding only
        /// sub-components has no bodies and no mass of its own, which is correct; a component
        /// WITH bodies reporting zero mass is Fusion's Inactive (Read-Only) failure, where
        /// physicalProperties returns 0 without raising. Without the count those are the same
        /// number and FusionStageService would have to reject both or accept both.
        ///
        /// Centre of mass is converted from Fusion's internal CENTIMETRES to metres here.
        /// Inertia is NOT converted -- its unit has not been verified against a hand-computed
        /// solid, and applying a factor that might be wrong is worse than labelling it.
        /// </summary>
...
def run(context):
    ...
    for i in range(design.allComponents.count):
        ...
        if ok:
            ...
            entry['inertiaUnits'] = 'UNVERIFIED - as returned by Fusion'
